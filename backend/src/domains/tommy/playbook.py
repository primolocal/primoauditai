...
def format_comment(template_key: str, **kwargs) -> str:
    """
    Format a Communication Playbook template with variables.

    Usage:
        format_comment("scan_invoice_missing", line=42)
        format_comment("oem_not_supported", part="hood", year=2022, mileage="34K")
    """
    template = RETURN_COMMENTS.get(template_key, RETURN_COMMENTS["safe_harbor"])
    try:
        return template.format(**kwargs)
    except KeyError:
        # Fallback: if template requires vars not provided, return raw template
        return template
